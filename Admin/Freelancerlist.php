...
if(isset($_GET['rem'])!=null)
{
	
	$delqry="delete from tbl_freelan where freelan_id='".$_GET['rem']."'";
	if($con->query($delqry))
	{
    ?>
    <script>
  ...
  <h2> Freelancer List</h2>
    <form id="frm_userlist" name="frm_list" method="post" action="">
      <table class="table table-bordered table-striped">
        ...
        <tbody>
          <?php 
            $selqry = "SELECT * FROM tbl_freelan WHERE freelan_status='1'";
          ...
          <tr>
            ...
            <td> <a href="Freelancerlist.php?rem=<?php echo $data['freelan_id'] ?>" class="btn btn-danger btn-sm">Remove</a></td>
          </tr>
          <?php } ?>
        </tbody>
      </table>
