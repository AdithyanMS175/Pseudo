...
if(isset($_GET['rem'])!=null)
{
	...
	$delqry="delete from tbl_client where client_id='".$_GET['rem']."'";
	if($con->query($delqry))
	{
    ?>
    <script>
  ...
  <h2> Client List</h2>
    <form id="frm_clientlist" name="frm_clientlist" method="post" action="">
      <table class="table table-bordered table-striped">
        ...
        <tbody>
          <?php 
            $selqry = "SELECT * FROM tbl_client WHERE client_status='1'";
          ...
          <tr>
            ...
            <td> <a href="Clientlist.php?rem=<?php echo $data['client_id'] ?>" class="btn btn-danger btn-sm">Remove</a></td>
          </tr>
          <?php } ?>
        </tbody>
      </table>
